...
vote_counts = vote_results_data_cleaned['vote'].copy(deep=True)
vote_counts = vote_results_data_cleaned['vote'].value_counts().sort_index()
...
percentage = ["{:.2f}%".format(x) for x in value_perc_vote.sort_index().values]
# Now, create the bar chart using Plotly
fig = px.bar(vote_counts, x='Vote', y='Count',
             labels={'Count': 'Number of occurences', 'Vote': 'Vote'},
...
# %%
#Look at the distribution of the values based on election outcome
...
fig = px.bar(x=results.index, y=results.values,
            ...
            title="Total number of votes based on election outcome",
            hover_data={"Percentage":percentage})
